...
const NoiseBackground = () => {
  // const [isVisible, setIsVisible] = useState(true); // isVisible 상태 제거
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const frameCountRef = useRef(0);
  const noiseTexturesRef = useRef<ImageData[]>([]); // noiseTextures를 ref로 관리

  // 저사양 기기 감지 로직 제거 (useEffect 훅 전체 삭제)

  useEffect(() => {
    // 노이즈 효과가 비활성화되면 더 이상 진행하지 않음 -> 이 조건 제거
    // if (!isVisible) return;
  ...
  }, []); // 의존성 배열에서 isVisible 제거

  // 노이즈 효과가 비활성화되면 아무것도 렌더링하지 않음 -> 이 조건 제거
